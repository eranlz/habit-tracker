    ...
    <div className="flex flex-col items-center gap-6">
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-xl font-bold text-white">
          {goal.isFailed ? "Marked as failed" : confirming ? "Are you sure?" : "On track!"}
        </p>
        <p className="text-sm text-white/50 text-center">
          {goal.isFailed
            ? "You can undo this if it was a mistake."
            : confirming
            ? "This will reset your streak and mark today as failed."
      ...
      {goal.isFailed ? (
        ...
        <div className="flex gap-3 w-full">
          ...
          <button
            ...
            className="flex-1 py-3 rounded-xl bg-danger/20 text-danger font-semibold hover:bg-danger/30 transition-colors flex items-center justify-center gap-2"
          >
            <XCircle size={18} /> Confirm
          </button>
        </div>
      ) : (
        <button
